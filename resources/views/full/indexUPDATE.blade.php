                ...
                @if (!Request::is('full_table*'))
                    <div class="row">
                        <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-12">
                                            <select name="select_bg" class="form-select select-bg">
                                                ...
                                                <option value="WB4" @selected(auth()->user()->default_wb === 'WB4' ? true : false)>Jembatan Timbang 4
                                                </option>
                                            </select>
                                        </div>
                                    </div>
                            ...
                            @elseif (auth()->user()->default_wb === 'WB4')
                                <div class="row align-items-center">
                                    <div class="col-lg-3 col-sm-12 row-btn-right">
                                        @if (auth()->user()->role == 'admin' || auth()->user()->role == 'spv')
                                            <a href="javascript:void(0)"
        ...
        <script>
            ...
            function wsRealTime() {
                ...
                ws.onmessage = function(e) {
                    ...
                    let truck_no = data.truck_no != null ? data.truck_no.replace(/\s/g, '').split(/(\/[A-Z])?/).filter(
                        Boolean) : null;
                    let audioFiles = [];
        ...
        <script>
            ...
            $(function() {
                ...
                $(document).on("click", ".btn-open-left", function(e) {
                    e.preventDefault();
                    let wb_left = $(this).data('wb');
                    let open_left = $(this).data('open_gate');

                    $('.savewb-form input[name=open_gate]').val(open_left);
                    $('.savewb-form input[name=wb]').val(wb_left);
                    $('#wb-form').modal('show');

                    $('.savewb-form').safeform({
                        timeout: 2000,
                        submit: function(event) {
                            event.preventDefault();
                            if (validateFormData($(this))) {
                                let open_gate = $(this).find('input[name=open_gate]').val(),
                                    wb = $(this).find('input[name=wb]').val(),
                                    plant_val = $(this).find('input[name=plant]').val(),
                                    seq_val = $(this).find('input[name=sequence]').val();

                                if (plant_val == '0' && seq_val == '0') {
                                    $.ajax({
                                        type: "post",
                                        ...
                                        success: function(res) {
                                            ...
                                            audioFiles.push(
                                                `/audio/open/wb${wb}-${open_gate}.mp3`
                                            );
                                            audioFiles.push(``);
                                        ...
                                        success: function(res) {
                                            if (res.status === 'success') {
                                                Swal.fire({
                                                    icon: 'success',
                                                    title: 'Success!',
                                                    text: `Jembatan timbang ${wb}, Open gate ${open_gate}`,
                                                }).then(function() {
                                                    $.ajax({
                                                        type: "post",
                                                        url: '{{ route('get_bearier') }}',
                                                        data: {
                                                            ...
                                                            sequence: getFieldValue(
                                                                'sequence',
                                                                formdata
                                                            ),
                                                            arrival_date: getFieldValue(
                                                                'arrival_date',
                                                                formdata
                                                            ),
                                                        },
                ...
                $(document).on("click", ".btn-open-right", function(e) {
                    e.preventDefault();
                    let wb_right = $(this).data('wb');
                    let open_right = $(this).data('open_gate');

                    $('.savewb-form input[name=open_gate]').val(open_right);
                    $('.savewb-form input[name=wb]').val(wb_right);
                    $('#wb-form').modal('show');
            ...
            function wsBarrierGate(WB = null) {
                ...
                let ws = new WebSocket(`${ws_url}/${WB}`);

                ws.onopen = function(event) {
                    console.log('Connection Barrier Gate Established');
                };

                let svgExample = "{{ asset('img/svg/barrier-gate.svg') }}";
                scadavisInit({
                    container: 'bg-svg',
